...
def _submit(key, token, filename):
    problem = get_problem(key)
    tests_result = []
    ...
    try:
        content = submit_code(token, key, open(filename).read(), tests_result)
    except:
        content = submit_code(token, key, "--exe--", tests_result)
...
def _usage():
    print('Usage:\n %s get <problem_key>\n %s submit <problem_key> <token> <filename>' % (sys.argv[0], sys.argv[0]))
    exit()
